...
################## Helper function
def get_factivity(list_file, gt_path, actions_dict):
    file_ptr = open(list_file, 'r')
    list_of_examples = file_ptr.read().split('\n')[:-1]
    file_ptr.close()

    out_dict, exist, total_v, activity_count = {}, {}, 0, {}
    sequence = {}
    for vid in list_of_examples:
        total_v +=1
        activity = vid.split('.txt')[0].split('_')[-1]
        if activity not in out_dict:
            out_dict[activity] = np.zeros(len(actions_dict))
        ...
        for c in contents:
            out_dict[activity][actions_dict[c]] +=1

        l, s, e = get_labels_start_end_time(contents)
        seq = ', '.join([i for i in l if i !='bg'] ) ## not count bg as it is not for temporal modelling
        if seq not in sequence[activity]:
            sequence[activity][seq] = 0
        sequence[activity][seq] += 1
    
    final_dict, activity2id, id2activity, id = {}, {}, {}, 0
    for key, value in out_dict.items():
        activity2id[key] = id
        id2activity[id] = key
        exist[id] = np.where(value>0)[0]
        p_activity = activity_count[key] / total_v
        final_dict[id] = p_activity * value / (np.sum(value))
        id += 1
    
    return final_dict, exist, activity2id, id2activity, sequence

def analyse_context(sequence_dict):
    activity_context, context = {}, {}
    for activity, seq_dict in sequence_dict.items():
        before_context, after_context = {}, {}
        for seq, cnt in seq_dict.items():
            seq_split = seq.split(', ')
            for i in range(len(seq_split)):
                if seq_split[i] not in before_context:
                    before_context[seq_split[i]] = {}
                    after_context[seq_split[i]] = {}
                
                af, bf = list(set(seq_split[i+1:])), list(set(seq_split[:i]))
                    
                for a in af:
                    if a == seq_split[i]:
                        continue 
                    if a not in after_context[seq_split[i]]:
                        after_context[seq_split[i]][a] = 0
                    after_context[seq_split[i]][a] += cnt
                    
                for b in bf:
                    if b == seq_split[i]:
                        continue 
                    if b not in before_context[seq_split[i]]:
                        before_context[seq_split[i]][b] = 0
                    before_context[seq_split[i]][b] += cnt

        must_context = {}
        for key in before_context.keys():
            must_bf = list(set(before_context[key].keys()) -  set(after_context[key].keys()))
            must_af = list(set(after_context[key].keys()) -  set(before_context[key].keys()))
            must_context[key] = (must_bf, must_af)
        
        activity_context[activity] = must_context
...
class MultiStageModel(nn.Module):
    def __init__(self, num_stages, num_layers, num_f_maps, dim, num_classes, nbins):
        ...
        self.stages = nn.ModuleList([copy.deepcopy(SingleStageModel(num_layers, num_f_maps, num_classes, num_classes))
                                     ...
                                     copy.deepcopy(SingleStageModel(num_layers, num_f_maps, num_classes, num_classes+nbins))
...
class Trainer:
    def __init__(self, model, log, sample_rate, **kwargs):
        ...
        self.creat_index()
        set_seed(seed)
        self.ndiff = self.total_num_out-num_classes
        self.model = model(nbins = self.ndiff, **kwargs)
        self.log = log
    ...
    def creat_index(self):
        ...
        for j in range(len(group_dict)):
            self.fg_splits.append(torch.from_numpy(np.array(group_dict[j])).long().to(device))

        binlabel_count = [1] * self.nbins
        label2binlabel = np.zeros((self.nbins, self.num_classes), dtype='int')
        for group, cid in enumerate(self.fg_splits):
            for c in cid:
                label2binlabel[group, c] = binlabel_count[group]
        ...
        self.total_num_out = np.sum(binlabel_count)


    def _remap_labels(self, labels, activity):
        ...
        for i in range(num_bins):
            if i != activity:
                new_bin_label = torch.zeros_like(labels).long().to(device)
            else:
    ...
    def suppression_loss(self, cls_score, labels, LA):
        '''
        :param cls_logits: shape TxC
        :param labels: shape T
        :return: softmax cross entropy based on modified logits
        '''
        def find_t(ls, s, max_flag):
            ls = self.label2binlabel[i][ls].cpu().numpy()
            if max_flag:
                t = []
                for k in s:
                    t.extend(list(np.where(ls==k)[0]))
                if len(t) ==0:
                    t1 = -1
                else:
                    t1 = np.max(t)
        ...
        total_loss = 0
        new_labels, new_weights, new_avgfactors = self._remap_labels(labels, LA)
        new_preds = self._slice_preds(cls_score)

        nums = 0
        for i in range(self.nbins):
            if i == LA:
                new_logit = new_preds[i]
                l, s, e = get_labels_start_end_time(labels.view(-1).cpu().numpy())

                assert np.min(activity_dist[i][self.fg_splits[i].cpu().numpy()]) > 0
                prior = torch.from_numpy(activity_dist[i][self.fg_splits[i].cpu().numpy()]).float()
                base = torch.max(prior)
                adjust = torch.ones_like(new_logit)*base
                for org_l in self.fg_splits[i].cpu().numpy():
                    new_l = self.label2binlabel[i][org_l].item()
                    if label_dict[org_l] not in activity_context[id2activity[i]]:
                        adjust[:,new_l] = activity_dist[i][org_l]
                        #print('not seen', org_l) # only for org_l = 0, namely bg class
                    else:
                        bf_context, af_context = activity_context[id2activity[i]][label_dict[org_l]]
                        bf_context, af_context = np.array([actions_dict[j] for j in bf_context]), np.array([actions_dict[j] for j in af_context])
                        bf_context, af_context = self.label2binlabel[i][bf_context], self.label2binlabel[i][af_context]
                        if len(bf_context) == 0:
                            t1 = 0
                        else:
                            t1 = 0
                            t = find_t(np.array(l), bf_context.cpu().numpy(), max_flag=1)
                            if t != -1:
                                t1 = e[t]
                        
                        if len(af_context) == 0:
                            t2 = len(labels.view(-1))
                        else:
                            t2 = len(labels.view(-1))
                            t = find_t(np.array(l), af_context.cpu().numpy(), max_flag=0)
                            if t != -1:
                                t2 = s[t]

                        adjust[t1:t2, new_l] = activity_dist[i][org_l]

                adjust = torch.log(adjust**args.tau).to(device)
                new_logit += adjust

                total_loss += self.loss_bins[i](new_logit, new_labels[i], new_weights[i], avg_factor=new_avgfactors[i])
                nums += 1
            else:
                total_loss += args.w *self.loss_bins[i](new_preds[i], new_labels[i], new_weights[i], avg_factor=new_avgfactors[i])
                nums += args.w
    ...
    def _merge_score(self, cls_score):
        ...
        for i, split in enumerate(self.fg_splits):
            activity_score.append(torch.mean(torch.log(fg_score[i][:, 0])).item())
        activity_id = np.argsort(np.array(activity_score))[0]

        fg_merge[:, self.fg_splits[activity_id]] = fg_score[activity_id][:, 1:]
        return fg_merge
    
    def _merge_score_true(self, cls_score, activity):
        '''
        Do softmax in each bin. Decay the score of normal classes
        with the score of fg.
        From v1.
        ...
        activity_id = activity2id[activity]
    ...
    def train(self, save_dir, num_epochs, batch_size, learning_rate, device, actions_dict,
        ...
        for epoch in range(resume_epoch, num_epochs):
            ...
            for _, items in enumerate(train_loader):
                ...
                mask_new = torch.ones((mask.shape[0], mask.shape[1] + self.ndiff, mask.shape[2])).to(device)
                ...
                for i, p in enumerate(ps):
                    if i != len(ps)-1:
                        ...
                        s_ce_loss = self.suppression_loss(p.transpose(2, 1).contiguous().view(-1, self.num_classes + self.ndiff),batch_target.view(-1), LA=activity2id[activity])                       
                        s_smooth_loss = 0.15 * torch.mean(torch.clamp(
                            self.mse(F.log_softmax(p[:, :, 1:], dim=1), F.log_softmax(p.detach()[:, :, :-1], dim=1)),
                ...
                merge_results = self._merge_score(ps[-1].transpose(2, 1).contiguous().view(-1, self.num_classes + self.ndiff))
        ...
        for i, items in enumerate(train_ad_loader):
            ...
            for i, p in enumerate(ps):
                if i != len(ps)-1:
                    ...
                    s_ce_loss = self.suppression_loss(p.transpose(2, 1).contiguous().view(-1, self.num_classes  + self.ndiff),
                                                      batch_target.view(-1), LA=activity2id[activity])                       
                    s_smooth_loss = 0.15 * torch.mean(torch.clamp(
                            self.mse(F.log_softmax(p[:, :, 1:], dim=1), F.log_softmax(p.detach()[:, :, :-1], dim=1)),
                            min=0, max=16) * mask_new[:, :, 1:])
                
                if i == len(ps) - 1:
                    merge_result_true = self._merge_score_true(ps[-1].transpose(2, 1).contiguous().view(-1, self.num_classes  + self.ndiff), activity)
                    s_ce_loss_split = -torch.log(merge_result_true)
                    s_ce_loss_split = s_ce_loss_split[torch.arange(len(batch_target.view(-1))), batch_target.view(-1)]
                    confidence = merge_result_true[torch.arange(len(batch_target.view(-1))), batch_target.view(-1)]
    ...
    def predict(self, model_dir, results_dir, features_path, batch_gen_tst, actions_dict, sample_rate, device):
        ...
        with torch.no_grad():
            ...
            for i, items in enumerate(test_loader):
                ...
                merge_results = self._merge_score(predictions[-1].transpose(2, 1).contiguous().view(-1, self.num_classes  + self.ndiff))
                _, predicted = torch.max(merge_results, 1)
...
parser.add_argument('--bg', default=1, type=int, help='0-do not change logit for auxillary class, 1 - add the largest adjustment to the aux class')
...
TYPE = '/mstcn_activity_mm_weighted{}_tla-tau{}-bg{}_{}'.format(args.w, args.tau, args.bg, args.seed)
...
log.logger.info("Training for MS-TCN with activity group  & multi-mode  & weighted and TLA")
...
activity_dist, group_dict, activity2id, id2activity, sequence_dict = get_factivity(vid_list_file, gt_path, actions_dict)
activity_context = analyse_context(sequence_dict)
...
freq_list = np.array([actions_dict[i] for i in lt_dict['frequent']])
common_list = np.array([actions_dict[i] for i in lt_dict['common']])
rare_list = np.array([actions_dict[i] for i in lt_dict['rare']])
...
if args.action == "train":
    if args.is_wandb:
        wandb.init(project='new_mstcn', entity='pang_neurips',
               name='mstcn_activity_mm_weighted{}_tla-tau{}-bg{}_{}_{}'.format(args.w, args.tau, args.bg, args.split, args.seed),  resume = (args.resume >0))
