...
class Trainer:
    def __init__(self, model, log, sample_rate, **kwargs):
        ...
        self.ldam = LDAMLoss(frame_per_cls, max_m=args.max_m, s=args.s).to(device)
        ...
        for epoch in range(resume_epoch, num_epochs):
            ...
            for _, items in enumerate(train_loader):
                ...
                for i, p in enumerate(ps):
                    if i == len(ps) - 1:
                        s_ce_loss = self.ldam(p.transpose(2, 1).contiguous().view(-1, self.num_classes), batch_target.view(-1))
                    else:
...
TYPE = '/mstcn_ldam-m{}-s{}{}_{}'.format(args.max_m, args.s, '-norm' if args.norm else '', args.seed)
...
log.logger.info("Training for MS-TCN with LDAM")
...
if args.action == "train":
    if args.is_wandb:
        wandb.init(project='new_mstcn', entity='pang_neurips',
               name='mstcn_ldam-m{}-s{}{}_split{}_{}'.format(args.max_m, args.s, '-norm' if args.norm else '', args.split, args.seed),  resume = (args.resume >0))
